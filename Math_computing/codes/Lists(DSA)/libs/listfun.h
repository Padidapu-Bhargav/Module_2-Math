...
avyuh *Mat_1(avyuh *B,double AB,double angle, int k);
avyuh *np_array(double a,double b);
...
avyuh *np_array(double a, double b){
	...
	c=createList(2,1);
	c->vector->data=a;
	c->next->vector->data=b;
	return c;
}

avyuh *Mat_1(avyuh *B, double AB, double angle, int k){
	avyuh *P=Listadd(B, np_array(AB*k*cos(angle), AB*sin(angle)));
	return P;
}
